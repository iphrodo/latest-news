...
#### Scenario: Category news item image fails to load in the browser
- **WHEN** a category news item has an image URL provided by the source, but the browser fails to load that image (e.g. the URL returns an error or is blocked by the image host)
- **THEN** the system shows the same placeholder used for items with no image URL, in place of the image on the left of the card, rather than showing a broken image
